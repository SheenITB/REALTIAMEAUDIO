...
if %PLATFORM% == "Win32" (
  ...
  if %FORMAT% == ".vst3" (
    ...
    xcopy /E /H /Y "%~dp0..\resources\web\*" "%BUILD_DIR%\%NAME%.vst3\"
    if exist %VST3_32_PATH% ( 
      ...
      xcopy /E /H /Y "%~dp0..\resources\web\*" "%VST3_32_PATH%\%NAME%.vst3\"
    )
  )
...
if %PLATFORM% == "x64" (
  ...
  if %FORMAT% == ".vst3" (
    ...
    if exist %VST3_64_PATH% (
      ...
      xcopy /E /H /Y "%~dp0..\resources\web\*" "%VST3_64_PATH%\%NAME%.vst3\"
    )
  )
